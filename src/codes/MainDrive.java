...
//	static 메쏘드에서도 사용하려고 static 키워드 첨부.
...
//		파일에서 내연락처를 불러올 예정.
//		myUserList 에 이미 연락처 데이터가 불러져 있는 상황이라면 [전부 다 지우고 ] 다시 불러오게 하자.
		myUserList.clear();
		
//		파일에 저장된 데이터  => 자바 프로그램에서 활용. (File INPUT)
//		FileReader / BufferedReader 활용
		
//		불러올 파일의 위치 지정 => 지정할 때 사용하는 파일명과 동일하게
...
//			모든 연락처를 불러올때까지 반복
			...
			while (true) {
//				한줄을 통째로 불러오기. => IOExecption 처리필요
				String line = br.readLine();
				
//				불러온 내용을 검사 . null인지
				if (line == null) {
//					더이상 불러올 내용이 없어서 null이 들어옴 => 다 읽었다.  => 무한반복 탈출
